...
The `osrlib` engine runs as a Model Context Protocol (MCP) server that Claude Code starts with the plugin. The engine keeps the whole game state and handles every mechanic: dice, to-hit rolls, saving throws, morale, initiative, XP, encumbrance, the map you've explored, character creation, and advancement. Claude doesn't do the math. It sends your action to the engine as a command, gets back what happened, and narrates. That's why the dice are real, the roll log is complete, and a saved game picks up exactly where you stopped.
