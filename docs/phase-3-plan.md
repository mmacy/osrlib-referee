...
- **On the play surface (live-session reads, small schemas):** `character_sheet(character_id)` and `session_audit(...)`. Plus the existing seven. That is the entire standing per-turn cost Phase 3 adds — two small tools — and it is accounted here, not hidden. The `AnyCommand` union still dominates the standing cost (Phase 1's finding); these two add little, but "little" is not "nothing," and the plan says so.
