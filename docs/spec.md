...
This exposes a gap: the command union has **no ad-hoc roll**. The rolling primitives exist in osrlib's core — `dice.roll`, `ability_check`, `saving_throw`, `check_morale`, each drawing from a seeded `RngStream` — but none is reachable as a command; the 44 are all specific keyed actions. Completing the authorial surface therefore needs a small **additive upstream osrlib command** — `RollDice`, and likely `Check`/`Save` — that draws from a named session stream, logs itself as an accepted command, and emits a result event to narrate from (the same pattern `SpawnMonsters` already follows when it rolls its count). This lands as **its own upstream osrlib PR** (plan → rubber-duck → PR, like the Phase 2a change) and is tracked in the roadmap. Until it lands, an uncertain outcome the engine cannot roll is resolved as a **declared referee ruling** committed via `SetFlag`, never a faked die. Constitution guardrails: no invented roll results, no out-of-band roller, no roll-shopping (re-rolling a seeded draw for a better number — the roll log makes it visible).
...
- **2c — the adventure-module-compiler skill.** Reads a published adventure module (PDF/MD) and emits the bundle: prose → sidecar; geometry → grid + `edges` (with a fidelity-loss log for non-grid maps and a `validate_adventure` + visual map-diff review gate); unique creatures → the injected catalog; puzzles/tricks → flagged for the escape hatch.
...
- **The adjudication-roll engine command (upstream osrlib PR).** Freeform adjudication of *chance* outcomes needs a seeded, logged roll, and the command union has none today. The fix is a small additive upstream command (`RollDice`/`Check`/`Save`) drawing from a named session stream and emitting a result event — opened as its own osrlib PR, like the 2a change (tracked upstream as [osrlib-python#22](https://github.com/mmacy/osrlib-python/issues/22)). It is additive and low-risk, so it can land early; until it does, unrollable uncertainty falls back to a declared `SetFlag` ruling, and only *freeform chance* adjudication is blocked (keyed content and referee rulings still work).
- **Geometry transcription cost.** Even automated, compiling a large adventure module's `edges` is the dominant Phase 2 effort and the likeliest place fidelity slips. Mitigate with a compile-time validator (`validate_adventure` already reports every problem at once) and a visual map-diff review step.
...
- Adjudication obeys one rule: every outcome enters the trajectory as a logged command, and any *chance* outcome (as opposed to a referee *judgment*) is rolled by the engine from the seeded stream — never invented, never out-of-band, never re-rolled for a better result. Because the command union has no ad-hoc roll, this requires an additive upstream osrlib command (`RollDice`/`Check`/`Save`), opened as its own PR; until then, unrollable uncertainty is a declared `SetFlag` ruling.
