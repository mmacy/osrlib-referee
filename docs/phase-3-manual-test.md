...
**Launch from the game directory, not the checkout.** That is how the plugin is installed for real, and it keeps the repo's `CLAUDE.md` — the contributor guide, all of it meta — out of the referee's context window. Every skill resolves the server through `${CLAUDE_PLUGIN_ROOT}`, so the `chargen`, `gametool`, and `bundletool` calls work from any working directory.
...
- A CLI call that fails to find a `pyproject.toml`, or a bare `server` path in the command — `${CLAUDE_PLUGIN_ROOT}` did not substitute, and the skills are no longer runnable outside the checkout.

## Act 10 — compile a module from the shelf

Not a Phase 3 act. This covers the shelf-of-holding text source and the `compile-adventure` stage 0 that landed after Phase 3 (#18), which none of the acts above touch. It needs no game session, so run it in its own sitting.

Target: **X1 The Isle of Dread**, the Taboo Island temple — areas 31–37 on level 1 and 38–40 on level 2. Ten keyed areas over two levels is small enough to finish in a sitting, and it still exercises a trap, a secret door, treasure, and creatures with no SRD template. X1 is a commercial TSR module, so it also puts the licensing gate under load.
...
- **Pass:** Before it compiles anything it establishes that X1 is a commercial TSR module and puts the bundle in `~/osr-games/bundles/<bundle_id>/`. Asking you to confirm the license is fine; silently compiling into the repo's `adventures/` is not.
...
- **Pass:** For Map 12 and Map 13 it reads the PDF page as an image (`Read` with `pages:`) and lays out the grid from that. Geometry does not survive OCR — the shelf text keeps a map's labels and drops its shape — so a grid derived from `shelf-text` output is wrong by construction, however plausible it looks.

### Act 10e — the two gates

- **Do:** Let it finish, then look at what it hands back.
- **Pass:** `bundletool validate` prints `OK`; it presents a `render-map` render next to the source map for the cell-for-cell diff; and `manifest.approximations` lists every reskin (the Great One and the guardians are the likely ones), every geometry approximation, and every escape-hatch beat, each with its cost stated rather than smoothed over.

### Act 10f — no index (optional)

- **Do:** `SHELF_DB=/nonexistent/shelf.db` in the environment, then ask for a compile.
- **Pass:** The command says `no shelf index at /nonexistent/shelf.db. Set SHELF_DB to one, or compile from text you supply instead.` and exits 1; the skill then asks you for the text rather than treating the missing index as a dead end.
